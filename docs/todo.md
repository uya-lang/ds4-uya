...
当前项目保持纯 Uya runtime，不再接入原 DS4 的 C runtime。Phase 9
已经完成 schema/tokenizer/dtype/Flash 原语和诊断覆盖，但 DS4 Flash Q2
生产模型的完整纯 Uya forward 仍未完成。真实 Q2 `generate/chat` 必须明确返回
unsupported-layout，而不是借助外部 C runtime 假装完成。
...
- [ ] 把 Flash MoE、split-LoRA attention、compressor/indexer、HC 串成完整 DS4 Flash Transformer layer forward，并接入纯 Uya `runtime_generate_to_buffer`。
...
- [x] 增加真实模型 schema smoke：`make flash-q2-smoke` 会跑 `audit`、`inspect`、`encode`、`format-chat`，并断言真实 Q2 GGUF 上 `generate/chat` 当前返回 rc=9 unsupported。
- [ ] 增加纯 Uya golden 对照：同 prompt 与可信参考实现对齐 logits top-k / token 序列，误差和可接受差异写入测试说明。
- [ ] 增加纯 Uya 性能验收：报告真实 DS4 Flash Q2 的 prompt/decode tokens/s、峰值内存、加载时间。
...
- 诊断能识别 IQ2 reference kernel、MoE/HC/compressor/indexer/split-LoRA 分支；真实生成路径当前保持 rc=9 unsupported，等待纯 Uya Flash layer 接入。
...
- `generate "hello"` rc=9，明确报 `unsupported model layout for current CPU forward path`。
- `chat` rc=9，原因同上；不会进入外部 C runtime。
- `make flash-q2-smoke DS4_FLASH_Q2_GGUF=/home/winger/ds4/gguf/DeepSeek-V4-Flash-IQ2XXS-w2Q2K-AProjQ8-SExpQ8-OutQ8-chat-v2.gguf` 作为纯 Uya schema smoke，应通过并验证 `generate/chat` 的 rc=9。
- `make flash-q2-perf` 当前明确失败并提示需要先完成纯 Uya Flash layer forward。
...
- [ ] `build/ds4-uya generate <ds4-flash-q2.gguf> "<prompt>"` 能用纯 Uya 加载完整模型并产出文本。
- [ ] `build/ds4-uya chat <ds4-flash-q2.gguf>` 使用 chat template，多轮输入不会破坏 KV/cache 状态或内存。
- [ ] DS4 Flash Q2 中出现的所有 tensor dtype 和结构分支都有纯 Uya 实现或明确跳过理由。
- [x] 对 unsupported/缺 tensor/坏 shape/坏 dtype 的报错能定位到具体 schema/tensor 分支；dense Uya path 和未完成 Flash layout 保持明确 rc=9 诊断。
- [ ] 真实模型纯 Uya smoke、golden 对照、性能 benchmark 纳入 documented test target。
