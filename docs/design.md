...
- GGUF tensor data 读取到 `TensorView.data`，并绑定进 forward。
...
`n_head == n_head_kv`、F32/F16 norm 与矩阵、Q8_0/Q4_K matvec。完整 DS4
MoE/GQA 生产布局仍需要继续扩展，但会明确返回 unsupported-layout 诊断，不再假装
生成。
