...
- `cli`: 命令行入口，负责 `inspect`、未来的 `generate`/`chat`。
...
- `bench` CLI 报告 synthetic prompt/decode tokens/s，先用于本机回归趋势，真实模型 benchmark 等 GGUF-backed generation 接入后再补。
...
当前已经实现到 CPU optimization/benchmark 阶段：
...
- F32/F16、RMSNorm、RoPE、Softmax、dense matvec、Q8_0/Q4_K dot、SiLU/SwiGLU 标量 reference kernel。
- dense F32 小模型 forward：超参、层权重绑定、single-token、逐 token prefill、KV cache、logits 输出。
...
- `generate`/`chat` CLI entrypoints，当前会明确提示完整 GGUF-backed generation 尚未接入。
...
- binary/GGUF/tokenizer/tensor/kernel/model/sampler/optimization 测试。

这不是完整 DS4 生成器。它是后续纯 Uya CPU 生成器的加载和验证基座。
